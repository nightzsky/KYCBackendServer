...
def decrypt_request(json):
    ...
    for key in json:
        if type(json[key]) == dict:
            decrypted[rsa_decrypt(java_to_python_bytes(ast.literal_eval(key)),private_key)] = decrypt_request(java_to_python_bytes(json[key]))
        else:
            decrypted[rsa_decrypt(java_to_python_bytes(ast.literal_eval(key)),private_key)] = rsa_decrypt(java_to_python_bytes(ast.literal_eval(json[key])), private_key)
